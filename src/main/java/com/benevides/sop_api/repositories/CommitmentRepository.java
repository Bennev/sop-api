...
public interface CommitmentRepository extends JpaRepository<Commitment, Long> {
    ...
    @Query("SELECT new com.benevides.sop_api.domain.commitment.GetCommitmentsWithPaymentCountDTO(c.id, c.date, c.value, c.note, c.commitment_number,COALESCE(COUNT(p), 0)) " +
            "FROM commitment c LEFT JOIN c.payments p " +
            "GROUP BY c.id")
    Page<GetCommitmentsWithPaymentCountDTO> findAllWithPaymentCountByExpenseId(Long expenseId, Pageable pageable);
